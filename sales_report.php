...
// Consulta SQL principal para obtener los sumatorios totalizados
$sql = "SELECT `POS group`, 
               SUM(CAST(REPLACE(`Total net revenue`, ',', '.') AS DECIMAL(10,2))) AS `Total net revenue`, 
               SUM(`Total VAT`) AS `Total VAT`, 
               SUM(`Total due amount`) AS `Total due amount`
...
$sql .= " GROUP BY `POS group` ORDER BY `POS group` LIMIT $items_per_page OFFSET $offset";
...
$count_sql = "SELECT COUNT(DISTINCT `POS group`) AS total FROM tickets_ventas WHERE 1=1";
...
<body>
    ...
    <img src="images/nav-back.png" alt="Atrás" style="width: 12px; height: auto; cursor: pointer;">
        ...
        <form method="GET" action="sales_report.php" class="filters row">
        ...
        <!-- Resúmen -->
        <h3 class="my-4">Totalisations</h3>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="4">Aucune totalisation trouvée.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
        ...
        <nav aria-label="Pagination">
            <ul class="pagination">
                <?php if ($page_number > 1): ?>
                    <li class="page-item"><a class="page-link" href="?page_number=1&fecha_inicio=<?= urlencode($fecha_inicio) ?>&fecha_fin=<?= urlencode($fecha_fin) ?>&pos_group=<?= urlencode($pos_group) ?>&items_per_page=<?= $items_per_page ?>">First</a></li>
                    <li class="page-item"><a class="page-link" href="?page_number=<?= $page_number - 1 ?>&fecha_inicio=<?= urlencode($fecha_inicio) ?>&fecha_fin=<?= urlencode($fecha_fin) ?>&pos_group=<?= urlencode($pos_group) ?>&items_per_page=<?= $items_per_page ?>">Prev</a></li>
                <?php endif; ?>

                <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
                    <li class="page-item <?= $i === $page_number ? 'active' : '' ?>">
                        <a class="page-link" href="?page_number=<?= $i ?>&fecha_inicio=<?= urlencode($fecha_inicio) ?>&fecha_fin=<?= urlencode($fecha_fin) ?>&pos_group=<?= urlencode($pos_group) ?>&items_per_page=<?= $items_per_page ?>"><?= $i ?></a>
                    </li>
                <?php endfor; ?>

                <?php if ($page_number < $total_pages): ?>
                    <li class="page-item"><a class="page-link" href="?page_number=<?= $page_number + 1 ?>&fecha_inicio=<?= urlencode($fecha_inicio) ?>&fecha_fin=<?= urlencode($fecha_fin) ?>&pos_group=<?= urlencode($pos_group) ?>&items_per_page=<?= $items_per_page ?>">Next</a></li>
                    <li class="page-item"><a class="page-link" href="?page_number=<?= $total_pages ?>&fecha_inicio=<?= urlencode($fecha_inicio) ?>&fecha_fin=<?= urlencode($fecha_fin) ?>&pos_group=<?= urlencode($pos_group) ?>&items_per_page=<?= $items_per_page ?>">Last</a></li>
                <?php endif; ?>
            </ul>
        </nav>
    </div>

    <!-- Inclusión de JS para Bootstrap (Opcional) -->
